...
function [idx,netsim,dpsim,expref,pref,lowp,highp]=apclusterK(...
    s,kk,prc,nrun,nconv,lam,splot,lowp,highp,lowk,highk);
...
while ~dn
    ...
    fprintf('  Trying p=%f\n',tmppref);
    [idx,netsim,dpsim,expref]=apcluster(S,tmppref,'dampfact',lam, ...
        'convits',nconv,'maxits',nrun,'nonoise');
...
if abs(tmpk-kk)/kk*100>prc
    ...
    while (abs(tmpk-kk)/kk*100>prc)&&(ntries<20)
        ...
        [idx,netsim,dpsim,expref]=apcluster(S,tmppref,'dampfact',lam, ...
            'convits',nconv,'maxits',nrun,'nonoise',splot);
        tmpk=length(unique(idx));
