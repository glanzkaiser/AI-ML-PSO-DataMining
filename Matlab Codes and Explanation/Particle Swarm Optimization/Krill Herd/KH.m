...
NR = 10;                                  % jumlah jika run
NK = 25; 						     	  % jumlah jika Krills
MI = 200; 		                          % Max iterasi
C_flag = 1;                               % Crossover flag [Yes=1]

% Bounds (normalisasi tiap space dr highly imbalanced search space)
UB = 10*ones(1,10);
LB = -10*ones(1,10);

NP = length(LB); %jumlah Parameter(s)
...
for nr = 1:NR
    ...
    for j = 1:MI 
        ...
        Xf(:,j) =findlimits(Xf(:,j)',LB,UB,Xgb(:,j,nr)');% check bounds
        Kf(j) = cost(Xf(:,j));
        if 2<=j
            if Kf(j-1)<Kf(j)
                Xf(:,j) = Xf(:,j-1);
        ...
        for i = 1:NK
            ...
            %kalkulasi efek NEIGHBORS KRILL 
            nn=0;
            ds = mean(R)/5;
            alpha_n = 0;
            for n=1:NK
