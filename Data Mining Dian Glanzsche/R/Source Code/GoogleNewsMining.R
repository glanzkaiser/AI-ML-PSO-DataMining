...
getNews <- function(symbol, number){
    ...
    require(xts); require(RDSTK); require(RCurl)
    
    # construct url to news feed rss and encode it correctly
    url.b1 = 'http://news.google.com/news?q='
...
getNews2 <- function(symbol, number){
    ...
    # construct url to news feed rss and encode it correctly
    url.b1 = 'http://news.google.com/news/feeds?q='
    url    = paste(url.b1, symbol, '&output=rss', sep = '')
    url    = URLencode(url)
