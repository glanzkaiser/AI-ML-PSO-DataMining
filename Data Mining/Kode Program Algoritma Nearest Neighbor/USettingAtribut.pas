...
type
  TFrmSettingAtribut = class(TForm)
    ...
    procedure EdtKetKeyPress(Sender: TObject; var Key: Char);
...
begin
  ...
  begin
    ...
    begin
      ...
      SubItems.Add(FieldByName('bobot').AsString);
      Next;
    end;
  end;
...
begin
  ...
  begin
    MessageDlg('Beri nama variabelnya!', mtInformation, [mbOK],0);
    CbAktif.SetFocus;
    Exit;
  end;
  with DM.Q1 do
  begin
      SQL.Text :=
        ...
        ' is_aktif = :is_aktif, ket = :ket, bobot = :bobot '+
        ' where nama_atribut = :namalama ';
      ParamByName('namalama').AsString := LVAtribut.Selected.Caption;
      ParamByName('ket').AsString := EdtKet.Text;
      ParamByName('bobot').AsFloat := StrToFloat(EdtBobot.Text);
...
begin
  ...
  begin
    MessageDlg('Karakter yang diinputkan tidak diijinkan '+
      ' sebagai karakter pertama nama variabel', mtInformation, [MBOK],0);
    key := char(0);
  end else if not
    (key in [#48..#57, #65 ..#90, #95, #97 ..#122, chr(VK_BACK)]) then
  begin
    MessageDlg('Karakter yang diinputkan tidak diijinkan dalam nama variabel',
...
procedure TFrmSettingAtribut.EdtKetKeyPress(Sender: TObject;
  var Key: Char);
begin
  if Key = #13 then CbAktif.SetFocus
end;
...
begin
  ...
  EdtKet.Text := LVAtribut.Selected.SubItems[1];
